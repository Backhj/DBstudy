...
-- 문자열 타입은 '' 숫자는 그냥 숫자로 하지만 '10'도 받아줌 
INSERT INTO ex3_1(col1, col2) -- 특정컬럼만 삽입할떄 무조건 컬럼명 작성
VALUES('judy',9);
INSERT INTO ex3_1(col1, col2) -- 특정컬럼만 삽입할떄 무조건 컬럼명 작성
VALUES('judy',20);
-- 2. 테이블에 있는 전체 컬럼에 대해서 삽입할때는 안써도 됨
...
from emp;
DELETE emp
WHERE empno = 200; -- dep 참조하고 있는 emp 데이터 부터 삭제후 삭제 가능
...
-- 표현식 (테이블에특정 데이터의 표현을 바꾸고 싶을때 사용)
...
FROM tb_info
-- WHERE nm LIKE '김%'; -- %= 모든것 
-- WHERE nm LIKE '%길'; -- %= 길로 끝나는 모든것
WHERE nm LIKE '%앞%'; -- %= 모든것 
-- td_info에서 email에서 naver포함된 학생을 조회
SELECT *
FROM tb_info
WHERE nm LIKE '%@naver.com'; -- %= 모든것 
...
SET col2 = 20      -- 수정하고자하는 컬럼과 데이터
    ,col3=SYSDATE  -- 수정해야하는 컬럼이 여러개라면 ,로 추가 
...
-- pos에 0 오면 1로 (디폴트1)
-- pos에 음수가 오면 문자열의 맨 끝에서 시작한 상대적 위치이다.
-- len이 생략되면 pos번째 부터 나머지 모든 문자를 반환한다.
SELECT SUBSTR('ABCK EFG', 1,4) as ex1
       ,SUBSTR('ABCK EFG', 4)  as ex2
       ,SUBSTR('ABCK EFG', -3,3) as ex3
FROM dual;

--INSTR 대상 문자열의 위치 찾기
-- 매개변수 총 4개 2개는 필수 뒤에는 디폴트 1,1
--(p1, p2, p3, p4)p1 대상문자열, p2 찾을 문자, p3 찾기 시작위치, p4 찾을 문자의 몇번째 출현
...
-- leeapgil@gamil.com ->> 아이디:leeapgil 도메인:gamil.com
  ...
  SELECT
    NM,
    EMAIL,
    SUBSTR(EMAIL, INSTR(EMAIL, '@') - 1) AS EMAIL_ID,
